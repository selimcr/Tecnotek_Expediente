...
class Export
{
    ...
    public function writeCellByPosition($row, $col, $data, $height = null, $backgroundColor = null, $rotation = null){
        $this->writeCellByPositionWithOptions($row, $col, $data, array());
    }

    public function writeCellByPositionWithOptions($row, $col, $data, $options){
        $this->cursor['x'] = $col;
        $this->cursor['y'] = $row;

        $this->currentSheet->setCellValueExplicitByColumnAndRow($col, $row, $data);

        // Options heightRow
        if (isset($options['height'])) {
            $this->currentSheet->getRowDimension($row)->setRowHeight($options['height']);
        }

        // Options heightRow
        if (isset($options['width'])) {
            $this->currentSheet->getColumnDimensionByColumn($col)->setWidth($options['width']);
        }
        ...
        if (isset($options['backgroundColor'])) {
            $this->currentSheet
                ...
                'fill' => array(
                    ...
                    'color' => array('rgb' => $options['backgroundColor']),
                )
            ));
        }

        if (isset($options['rotation'])) {
            $this->currentSheet->getStyleByColumnAndRow($col, $row)->getAlignment()->setTextRotation($options['rotation']);
        }
    ...
    public function applyBorderByRange($initialCol, $initialRow, $finalCol, $finalRow){
        $styleArray = array(
            'borders' => array(
                'allborders' => array(
        ...
        for($r = ""; $initialCol >= 0; $initialCol = intval($initialCol / 26) - 1)
            $r = chr($initialCol%26 + 0x41) . $r;

        $range = $r . "" . $initialRow;

        for($r = ""; $finalCol >= 0; $finalCol = intval($finalCol / 26) - 1)
            $r = chr($finalCol%26 + 0x41) . $r;

        $range .= ":" . $r . "" . $finalRow;

        $this->currentSheet->getStyle($range)->applyFromArray($styleArray);
    }
